...
const uploadChat = multer({ storage: new CloudinaryStorage({ cloudinary: cloudinary, params: { folder: 'aurora_chat', allowed_formats: ['jpg', 'png', 'jpeg', 'gif', 'webp'] } }) });
...
const MessageSchema = new mongoose.Schema({
  ...
  avatar: { type: String }, text: { type: String }, time: { type: String }, timestamp: { type: Date, default: Date.now }
});
...
io.on('connection', (socket) => {
  ...
  // ✏️ PACK A : ÉDITION ET SUPPRESSION
  ...
  // 📞 PACK C : FIX WEBRTC ET SYNCHRONISATION
  ...
  socket.on('end-call', (room) => socket.to(room).emit('end-call')); // Synchronise le raccrochage
